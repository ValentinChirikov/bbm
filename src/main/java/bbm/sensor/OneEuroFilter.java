 ...
 * First beta is set to 0 and fcmin (mincutoff) to a reasonable middle-ground value such as 1 Hz.
 * Then the body part is held steady or moved at a very low speed while fcmin is adjusted
 * to remove jitter and preserve an acceptable lag during these slow movements
 * (decreasing fcmin reduces jitter but increases lag, fcmin must be > 0).
 ...
 * You can first multiply and divide beta by factor 10 until you notice an effect on latency when moving quickly.
 * Note that parameters fcmin and beta have clear conceptual relationships:
